...
开始页有五个入口：**规则介绍**、**入门玩法**（3 关单格箭，整盘铺满一半以上、从零练手）、
**基础玩法**（12 关）、**字母玩法**（26 个字母各一关，整盘铺成那个字母）、**随机关卡**
（随机造型现场生成）。
...
| 快捷键 | `U` 撤销 · `H` 提示 · `A` AI 自动求解 · `G` 辅助线 · `T` 日夜主题 · `Esc` 菜单与返回 |
...
| 日间主题的规则页（按键表里那个小拨杆：滑块在左 = 日间） | 日间主题的开始页 |
|---|---|
| ![日间规则](assets/screenshots/rules_day.png) | ![日间开始](assets/screenshots/start_day.png) |
